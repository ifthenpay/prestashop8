...
class RememberController extends ModuleAdminController
{
    ...
    public function postProcess()
    {
        try {
            ...
            Utility::setPrestashopCookie('success', $this->module->l('Remember notification sent with success!', pathinfo(__FILE__)['filename']));
        } catch (Exception $th) {
            IfthenpayLogProcess::addLog('Error sending remember notification - ' . $th->getMessage(), IfthenpayLogProcess::ERROR, $order->id);
            Utility::setPrestashopCookie('error', $this->module->l('Error sending remember notification!', pathinfo(__FILE__)['filename']));
        }
