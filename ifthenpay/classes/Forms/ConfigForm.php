...
abstract class ConfigForm
{
	...
	protected function getCommonConfigFormValues()
	{
		...
		return [
			'IFTHENPAY_CALLBACK_ACTIVATE_FOR_' . $paymentMethod => \Configuration::get('IFTHENPAY_CALLBACK_ACTIVATED_FOR_' . $paymentMethod, false),
			'IFTHENPAY_PAYMENT_METHOD' => $this->paymentMethod,
			'IFTHENPAY_' . $paymentMethod . '_MAXIMUM' => \Configuration::get('IFTHENPAY_' . $paymentMethod . '_MAXIMUM', false),
	...
	protected function addEnableConfirmedOrderStatusWithInvoiceToForm()
	{
		$this->form['form']['input'][] = [
			'type' => 'switch',
			'label' => $this->ifthenpayModule->l('Invoice', pathinfo(__FILE__)['filename']),
			'name' => 'IFTHENPAY_' . strtoupper($this->paymentMethod) . '_INVOICE',
			'desc' => $this->ifthenpayModule->l('Enable this Payment Method\'s Invoice option for it\'s confirmed order status. If enabled, Prestashop will execute the invoice logic when an order payment is confirmed.'),
			'is_bool' => true,
			'values' => [
				[
					'id' => 'active_on',
	...
	protected function deleteCommonConfigValues()
	{
		...
		\Configuration::deleteByName('IFTHENPAY_' . strtoupper($this->paymentMethod . '_INVOICE'));
		\Configuration::deleteByName('IFTHENPAY_' . strtoupper($this->paymentMethod));
		\Configuration::deleteByName('IFTHENPAY_ACTIVATE_NEW_' . strtoupper($this->paymentMethod . '_ACCOUNT'));
	}
	...
	protected function updatePaymentMethodConfirmedOrderStatus()
	{
		$paymentMethod = strtoupper($this->paymentMethod);

		$storedHasInvoice = \Configuration::get('IFTHENPAY_' . $paymentMethod . '_INVOICE') ? '1' : '0';
		$hasInvoice = \Tools::getValue('IFTHENPAY_' . $paymentMethod . '_INVOICE') ? '1' : '0';

		if ($storedHasInvoice != $hasInvoice) {

			\Configuration::updateValue('IFTHENPAY_' . $paymentMethod . '_INVOICE', $hasInvoice);

			$statusId = \Configuration::get('IFTHENPAY_' . $paymentMethod . '_OS_CONFIRMED');

			IfthenpayModelFactory::build($this->paymentMethod)->updateOrderStatusInvoice($statusId, $hasInvoice);
		}
	}
