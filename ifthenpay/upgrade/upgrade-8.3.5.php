...
/**
 * File: /upgrade/upgrade-8.3.5.php
 ...
 * @return void
 */
function upgrade_module_8_3_5($module)
{
	// Process Module upgrade to 8.3.5

	IfthenpayLogProcess::addLog('Running module upgrade to version 8.3.5', IfthenpayLogProcess::INFO, 0);
	...
	if (
		...
		IfthenpayLogProcess::addLog('Ran upgrade script 8.3.5 (disableCofidis() & reactivateCallbacks() & updateUserAccounts) with result success', IfthenpayLogProcess::INFO, 0);
		return true;
	}
	return false;
}
...
function reactivateCallbacks(): bool
{
	try {
		...
		IfthenpayLogProcess::addLog('Partial Fail to run upgrade script 8.3.5 with info message: Failed to reactivate callbacks' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
...
function updateUserAccounts()
{
	try {
		...
		IfthenpayLogProcess::addLog('Partial Fail to run upgrade script 8.3.5 with info message: Failed to update user accounts' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
...
function disableCofidis()
{
	...
	try {
		...
		IfthenpayLogProcess::addLog('Fail to run upgrade script 8.3.5 with info message: Failed to disable Cofidis payment method' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
		return false;
	}
}