...
/**
 * File: /upgrade/upgrade-8.3.0.php
...
{
	// Process Module upgrade to 8.3.0

	IfthenpayLogProcess::addLog('Running module upgrade to version 8.3.0', IfthenpayLogProcess::INFO, 0);
	...
	$statusArray = [
		...
		[
			...
			'statusNameArray' => [
				'pt' => 'Pagamento por Cofidis Pay não aprovado',
				'en' => 'Cofidis Pay payment not approved',
			],
			'sendEmail' => false,
			'template' => '',
			'color' => '#E74C3C',
	...
	$resultOfUpdateOrderStateNames = updateOrderStateNames();
	...
	if (
		...
		$resultOfUpdateOrderStateNames == true
	) {
		IfthenpayLogProcess::addLog('Ran upgrade script 8.3.0 (addOrderStatuses() & reactivateCallbacks() & updateUserAccounts) with result success', IfthenpayLogProcess::INFO, 0);
		return true;
	}
	return false;
}
...
function addOrderStatuses(array $statusArray): bool
{
	try {
			...
			if (!\Configuration::get($status['configName']) || !\Validate::isLoadedObject(PrestashopModelFactory::buildOrderState(\Configuration::get($status['configName'])))) {
				...
				foreach (\Language::getLanguages() as $language) {
					...
					if ($isoCode == 'en') {
						...
						$order_state->name[$language['id_lang']] = $status['statusNameArray']['en'];
						IfthenpayLogProcess::addLog('Unsupported language of ' . $isoCode . ', will set order status text to default english (upgrade script 8.3.0)', IfthenpayLogProcess::INFO, 0);
					}
				}
		...
		IfthenpayLogProcess::addLog('Fail to run upgrade script 8.3.0 with error message: ' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
...
function reactivateCallbacks(): bool
{
	try {
		...
		IfthenpayLogProcess::addLog('Partial Fail to run upgrade script 8.3.0 with info message: Failed to reactivate callbacks' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
...
function updateUserAccounts()
{
	try {
		...
		IfthenpayLogProcess::addLog('Partial Fail to run upgrade script 8.3.0 with info message: Failed to update user accounts' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
...
function createDatabaseTables()
{
	...
	try {
		...
		IfthenpayLogProcess::addLog('Fail to run upgrade script 8.3.0 with info message: Failed to create database tables' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
		return false;
	}
}
...
function updateOrderStateNames()
{
	$orderStates = [
		[
			'old' => 'Awaiting for Multibancopayment',
			'new' => 'Awaiting for Multibanco payment'
		],
		[
			'old' => 'Payment by Multibancoconfirmed',
			'new' => 'Payment by Multibanco confirmed'
		],
		[
			'old' => 'Awaiting for Ccardpayment',
			'new' => 'Awaiting for Credit Card payment'
		],
		[
			'old' => 'Payment by Ccardconfirmed',
			'new' => 'Payment by Credit Card confirmed'
		],
		[
			'old' => 'Awaiting for Cofidispaypayment',
			'new' => 'Awaiting for Cofidis Pay payment'
		],
		[
			'old' => 'Payment by Cofidispayconfirmed',
			'new' => 'Payment by Cofidis Pay confirmed'
		],
		[
			'old' => 'Awaiting for Mbwaypayment',
			'new' => 'Awaiting for MB WAY payment'
		],
		[
			'old' => 'Payment by Mbwayconfirmed',
			'new' => 'Payment by MB WAY confirmed'
		],
		[
			'old' => 'Awaiting for Payshoppayment',
			'new' => 'Awaiting for Payshop payment'
		],
		[
			'old' => 'Payment by Payshopconfirmed',
			'new' => 'Payment by Payshop confirmed'
		],
	];
	...
	try {
		foreach ($orderStates as $orderState) {
			...
			$sql = 'UPDATE `' . _DB_PREFIX_ . 'order_state_lang`
			SET `name` = \'' . $orderState['new'] . '\'
			WHERE `name` = \'' . $orderState['old'] . '\'';

			$result = \Db::getInstance()->execute($sql);

			if ($result != true) {
				IfthenpayLogProcess::addLog('Partial Fail to run upgrade script 8.3.0 with info message: Failed to update order_state_lang names' . $th->getMessage(), IfthenpayLogProcess::INFO, 0);
			}
		}
	} catch (\Throwable $th) {
		return true; // intentional return true, it is expected to succeed update even if not able to update order_state_lang names
	}
	return true;
}
